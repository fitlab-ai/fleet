...
- 使用当前宿主实际加载的 hook/profile，并核对实际 parent/child 身份与终态；prepare 不依赖历史 capability 或 controller attestation。
